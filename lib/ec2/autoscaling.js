...
exports.scale = function(asgs, numberOrImpact, opts, callback) {
    ...
    // If we got a selector, we need to get resources for this
    return utils.when ( typeof asgs == "object" && asgs._selector != null,
        ...
        function(){
            // Scale every ASG
            return async.map(asgs,
                function(asg, next){
                    if ( asg._type != "ASG" )
                        return next(null, null);

                    // Scale!
                    console.log("Scaling "+asg.AutoScalingGroupName+" ...");
                    return _scale.call(self, asg, numberOrImpact, opts, function(err, res){
                        if ( err ) {
                            console.log("Error scaling ASG "+asg.AutoScalingGroupName+": ", err);
...
function _scale(asg, numberOrImpact, opts, callback) {
    ...
    // Get module set up for the requested region
    autoscaling = self.aws.getRegionModule(asg._region, 'AutoScaling');
    ...
    console.log("Scaling "+asgNames.length+" ASG's at "+asg._region+" to "+numberOrImpact);
    ...
    return utils.when(isImpact && needToFetch.length > 0,
        function(next) {
            ...
            // Describe autoscaling groups
            return self.getAutoscalingGroupsByName(asg._region, needToFetch, function(err, asgList) {
                if ( err ) {
        ...
        function() {
            ...
            async.map(asgNames,
                function(asgName, next){
                    ...
                    console.log("Setting ASG '"+asgName+"' desired capacity on "+asg._region+" to "+desiredByASG[asgName]+"...");
